#!/bin/bash
#SBATCH --job-name=edain_pn_noclip_f${FOLD:-0}
...
# EDAIN: anchor=population_nyul + outlier_clip=none  (= old Run 1, was 0.7389)
set -euo pipefail
module purge
cd "$SLURM_SUBMIT_DIR"
mkdir -p logs
...
echo "[EDAIN pn+noclip] fold=$FOLD | start $(date)"
nnUNetv2_train $DATASET_ID $CONFIG $FOLD -tr nnUNetTrainerEDAIN --npz
echo "[EDAIN pn+noclip] fold=$FOLD | end $(date)"
