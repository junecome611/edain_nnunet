#!/bin/bash
#SBATCH --job-name=nyul_identity_val_f${FOLD:-0}
...
# ============================================================
# EXPERIMENT 3 (validation-only): Nyul-identity.
#
...
# (val cases now precomputed; case_id now plumbed during sliding-window
# inference). Should produce a noticeably higher Mean Validation Dice than
# the original buggy run.
#
# REQUIRES
#     $nnUNet_results/Dataset500_Lipo/nnUNetTrainerNyulIdentity__nnUNetPlans__3d_fullres/
...
#     The old (buggy) numbers are lost, so back them up first if you want to
#     compare side-by-side:
#         cp $output_folder/validation/summary.json /backup/nyul_identity_buggy.json
# ============================================================
...
CKPT="$nnUNet_results/Dataset500_Lipo/nnUNetTrainerNyulIdentity__nnUNetPlans__${CONFIG}/fold_${FOLD}/checkpoint_final.pth"
if [ ! -f "$CKPT" ]; then
    echo "ERROR: $CKPT not found — was training finished?"
    exit 1
fi
echo "[exp 3: Nyul identity, val-only] fold=$FOLD using $CKPT"
echo "[exp 3: Nyul identity, val-only] start $(date)"

# --val:    run only perform_actual_validation, loading checkpoint_final.pth
# (incompatible with --c; the checkpoint must already exist)
nnUNetv2_train $DATASET_ID $CONFIG $FOLD \
    -tr nnUNetTrainerNyulIdentity \
    --val --npz

echo "[exp 3: Nyul identity, val-only] fold=$FOLD end $(date)"
